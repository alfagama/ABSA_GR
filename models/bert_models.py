...
def bert_methods():
    ...
    for i, item in enumerate(train_list):
        ...
        bert = TFAutoModel.from_pretrained("nlpaueb/bert-base-greek-uncased-v1")
        #
        df = pd.read_csv(train_list[i], sep=',')
        df_test = pd.read_csv(test_list[i], sep=',')
        #
        print(item)
        print(train_list[i])
        print(len(df))
        ...
        #
        # classifier head
        x1 = tf.keras.layers.Dense(512, activation='relu')(embeddings)
        # x2 = tf.keras.layers.Dense(258, activation='relu')(x1) 
        y = tf.keras.layers.Dense(5, activation='softmax', name='outputs')(x1)
        #
